...
public class ProductService {
    ...
    @Transactional
    public String createProduct(Product product, MultipartFile imageFile) throws IOException {
        try {
    ...
    private ProductDto convertToProductDto(Product product) {
        ...
        if (reviewDtos != null && !reviewDtos.isEmpty()) {
            double averageRating = reviewDtos.stream()
                    ...
                    .average()
                    .orElse(0.0);
            productDto.setAverageRating(Math.round(averageRating * 10.0) / 10.0);
        } else {
        ...
        if (productRepository.existsById(product.getId())) {
            try {
                ...
                if (existingProductOptional.isPresent()) {
                    ...
                    if (imageFile != null && !imageFile.isEmpty()) {
                        String imagePath = saveUploadedFile(imageFile);
                        productToBeUpdated.setImage(imagePath);
                    ...
                    productToBeUpdated.setSales(product.getSales());
